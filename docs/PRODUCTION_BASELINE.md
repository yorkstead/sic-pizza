...
The ADR-0001 implementation now binds server services to authenticated organization/location context, removes manufactured/default scope from PostgreSQL reads, commits state with audit and outbox records through one repository transaction, and adds migration `0003_premium_sentinel.sql` for non-null ownership and uniqueness constraints. The separate `test:integration` command passed four repository scenarios against an ephemeral local PostgreSQL 17 database on 2026-09-01. The container and synthetic data were removed afterward. The existing Neon `DATABASE_URL` was not migrated, seeded, or tested.

Endpoint-level verification now lives in `tests/integration/persisted-api-workflow.integration.ts` and runs through `bun run test:workflow`. On 2026-09-01 it passed against a separate ephemeral PostgreSQL 17 container and a production Next.js server using synthetic secrets. Independent staff and guest HTTP clients completed staff authentication, table opening, signed-token guest joining, guest proposal, staff approval, kitchen firing, server restart, persisted-state reload, and identical idempotent kitchen-fire replay with the original ticket ID. The harness also exposed and corrected domain/database ID drift by using canonical UUIDs for new runtime entities. Its temporary server, container, and synthetic data were removed afterward.

This is local workflow evidence, not deployment or pilot evidence. Staff authentication still uses the repository's seeded demo directory, realtime remains process-local, payments remain sandboxed, and no physical printer/reader behavior was exercised. Neon, hosting, routing, and live domains were not changed or verified.
